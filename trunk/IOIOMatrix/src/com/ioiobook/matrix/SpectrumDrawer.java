...
public class SpectrumDrawer {
	...
	// 128 values in average_ we just want 8 - Fn = n * Fs / N where Fn is freq at 
	...
	public SpectrumDrawer(int[][] display) {
		...
		win_ = new Window(MainActivity.AUDIO_BUFFER_SIZE, Window.Function.BLACKMAN_HARRIS);
		spectrumAnalyser_ = new FFTTransformer(MainActivity.AUDIO_BUFFER_SIZE, win_);
	...
	public void calculateSpectrum(short[] buffer) {
		// apply FFT to the buffer to get the spectrum, but we only have 8 columns
		// so sum into 8 bands
		spectrumAnalyser_.setInput(buffer, 0, MainActivity.AUDIO_BUFFER_SIZE);
		spectrumAnalyser_.transform();
		historyIndex_ = spectrumAnalyser_.getResults(average_, histories_, historyIndex_);
